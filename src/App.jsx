...
/* ============================================================
   CHAUDHARY TRADERS — Browser ERP (Phase 1 + Phase 2)
...
function Btn({ children, onClick, variant = "primary", type = "button", small }) {
  const base = `${small ? "px-3 py-1.5 text-xs" : "px-4 py-2 text-sm"} font-bold uppercase tracking-wide transition-colors`;
  const styles = {
...
function computeLedgerForCustomer(customer, invoices, payments, returns = [], exchanges = []) {
  const entries = [];
  invoices
  ...
  payments
    ...
    .forEach((p) =>
      entries.push({ date: p.date, type: "Payment", ref: p.method || "Cash", debit: 0, credit: p.amount, id: p.id })
    );
  returns
  ...
  exchanges
    .filter((ex) => ex.customerId === customer.id)
    .forEach((ex) => {
      if (ex.difference > 0) {
...
function Dashboard({ customers, invoices, payments, returns, exchanges, leads, bookings }) {
  const outstandingTotal = useMemo(() => {
    return customers.reduce((sum, c) => sum + computeLedgerForCustomer(c, invoices, payments, returns, exchanges).outstanding, 0);
  }, [customers, invoices, payments, returns, exchanges]);
...
function Customers({ customers, invoices, payments, returns, exchanges, saveCustomer, deleteCustomer, openLedger, branches, currentUser }) {
  ...
  const rows = customers
    ...
    .map((c) => {
      const { outstanding } = computeLedgerForCustomer(c, invoices, payments, returns, exchanges);
...
function LedgerView({ customers, invoices, payments, returns, exchanges, focusId, setFocusId, settings }) {
  ...
  const { entries, outstanding } = computeLedgerForCustomer(customer, invoices, payments, returns, exchanges);
...
function InvoiceForm({ customers, products, drivers, bookings, invoices, payments, returns, exchanges, prefill, editingInvoice, currentUser, onSave, onCancel, nextNumber }) {
  ...
  const previousOutstanding = isEdit
    ...
    : selectedCustomer
    ? computeLedgerForCustomer(selectedCustomer, invoices, payments, returns, exchanges).outstanding
    : 0;
...
function InvoiceDetail({ invoice, settings, onClose, onEdit, onCancelInvoice }) {
...
function Invoices({ customers, products, drivers, invoices, payments, returns, exchanges, bookings, settings, currentUser, saveInvoice, updateInvoice, cancelInvoice, prefill, onClearPrefill, onBookingFulfilled, onOrderFulfilled }) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-slate-200">
              ...
              <th className="px-4 py-2 text-right">Total</th><th className="px-4 py-2 text-right">Due</th><th className="px-4 py-2">Status</th>
            </tr>
          </thead>
          <tbody>
            {sorted.length === 0 && <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400">Koi invoice nahi bana.</td></tr>}
            {sorted.map((inv) => (
...
function InvoiceHistoryPage({ invoices }) {
  const rows = [];
...
function SalesReturnPage({ customers, invoices, returns, onCreateReturn }) {
  ...
  const returnAmount = invoice
    ? invoice.items.reduce((sum, it, idx) => sum + (Number(returnQtys[idx]) || 0) * it.price, 0)
    : 0;

  function submit() {
    if (!invoice) { alert("Pehle invoice select karein."); return; }
    const items = invoice.items
      .map((it, idx) => ({ name: it.name, unit: it.unit, qtyReturned: Number(returnQtys[idx]) || 0, price: it.price, total: (Number(returnQtys[idx]) || 0) * it.price }))
    ...
    onCreateReturn({
      ...
      date: todayISO(), items, reason: reason.trim(), amount: returnAmount,
    });
    setInvoiceId(""); setReturnQtys({}); setReason("");
  }
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-slate-200 p-4 mb-6 max-w-2xl">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Invoice">
            <select className={inputCls} value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)}>
              ...
              {customerInvoices.map((i) => <option key={i.id} value={i.id}>{i.number} — {fmtDate(i.date)} — {fmtMoney(i.total)}</option>)}
            </select>
          </Field>
        </div>
        ...
        {invoice && (
          <div className="mt-2">
            <div className="text-[11px] uppercase tracking-wide font-bold text-slate-500 mb-1">Items — Return Qty Daalein</div>
            <div className="space-y-2">
              {invoice.items.map((it, idx) => (
                <div key={idx} className="flex gap-2 items-center border border-slate-200 p-2">
                  <div className="flex-1 text-sm">
                    <div className="font-bold">{it.name}</div>
                    <div className="text-xs text-slate-400">Original Qty: {it.qty} {it.unit} · Rate: {fmtMoney(it.price)}</div>
                  </div>
                  <input
                    ...
                    onChange={(e) => setReturnQtys({ ...returnQtys, [idx]: Math.min(Number(e.target.value) || 0, it.qty) })}
                  />
                  <div className="w-24 text-right text-sm font-bold">{fmtMoney((Number(returnQtys[idx]) || 0) * it.price)}</div>
                </div>
              ))}
            </div>
      ...
      <div className="bg-white border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-slate-200">
            <th className="px-4 py-2">Return #</th><th className="px-4 py-2">Customer</th><th className="px-4 py-2">Invoice</th>
            <th className="px-4 py-2">Date</th><th className="px-4 py-2 text-right">Amount</th><th className="px-4 py-2">Reason</th>
          </tr></thead>
          <tbody>
            {sorted.length === 0 && <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400">Koi return record nahi.</td></tr>}
            {sorted.map((r) => (
...
function ExchangePage({ customers, products, invoices, exchanges, onCreateExchange }) {
  const [customerId, setCustomerId] = useState(customers[0]?.id || "");
  const [invoiceId, setInvoiceId] = useState("");
  const [returnQtys, setReturnQtys] = useState({});
  ...
  function submit() {
    if (!invoice) { alert("Pehle invoice select karein."); return; }
    const returnedItems = invoice.items
      .map((it, idx) => ({ name: it.name, unit: it.unit, qty: Number(returnQtys[idx]) || 0, price: it.price, total: (Number(returnQtys[idx]) || 0) * it.price }))
      .filter((it) => it.qty > 0);
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-slate-200 p-4 mb-6 max-w-3xl">
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Original Invoice">
            <select className={inputCls} value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)}>
              <option value="">Select Invoice</option>
              {customerInvoices.map((i) => <option key={i.id} value={i.id}>{i.number} — {fmtDate(i.date)}</option>)}
            </select>
          </Field>
        </div>
        ...
        {invoice && (
          <div className="mt-2">
            <div className="text-[11px] uppercase tracking-wide font-bold text-slate-500 mb-1">Customer Kya Wapis Kar Raha Hai</div>
            <div className="space-y-2 mb-3">
              {invoice.items.map((it, idx) => (
                <div key={idx} className="flex gap-2 items-center border border-slate-200 p-2">
                  ...
                  <input type="number" min="0" max={it.qty} className={`${inputCls} w-24`} placeholder="Return Qty"
                    value={returnQtys[idx] || ""}
                    onChange={(e) => setReturnQtys({ ...returnQtys, [idx]: Math.min(Number(e.target.value) || 0, it.qty) })} />
                  <div className="w-24 text-right text-sm font-bold">{fmtMoney((Number(returnQtys[idx]) || 0) * it.price)}</div>
                </div>
      ...
      <div className="bg-white border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-slate-200">
            <th className="px-4 py-2">Exchange #</th><th className="px-4 py-2">Customer</th><th className="px-4 py-2">Invoice</th>
            <th className="px-4 py-2">Date</th><th className="px-4 py-2 text-right">Difference</th><th className="px-4 py-2">Reason</th>
          </tr></thead>
          <tbody>
            {sorted.length === 0 && <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-400">Koi exchange record nahi.</td></tr>}
            {sorted.map((ex) => (
          ...
          <tbody>
            ...
            {sorted.map((cn) => (
              <tr key={cn.id} className="border-t border-slate-100 align-top">
                ...
                <td className="px-4 py-2">
                  <span className="text-[10px] font-bold uppercase px-2 py-0.5 bg-emerald-100 text-emerald-700">{cn.status}</span>
...
function Payments({ customers, payments, savePayment }) {
  const [form, setForm] = useState({ customerId: customers[0]?.id || "", date: todayISO(), amount: "", method: "Cash", note: "" });
  ...
  function submit() {
    ...
    savePayment({ id: uid("pay"), ...form, amount: Number(form.amount), customerName: customer.name });
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="text-left text-[11px] uppercase tracking-wide text-slate-500 border-b border-slate-200">
            <th className="px-4 py-2">Date</th><th className="px-4 py-2">Customer</th><th className="px-4 py-2">Method</th><th className="px-4 py-2 text-right">Amount</th>
          </tr></thead>
          <tbody>
            {sorted.length === 0 && <tr><td colSpan={4} className="px-4 py-6 text-center text-slate-400">Koi payment record nahi.</td></tr>}
            {sorted.map((p) => (
...
function Reports({ customers, invoices, payments, returns, exchanges, leads, bookings }) {
...
function SalesAssistant({ customers, invoices, payments, returns, exchanges }) {
  const [customerId, setCustomerId] = useState(customers[0]?.id || "");
  ...
  const outstanding = customer ? computeLedgerForCustomer(customer, invoices, payments, returns, exchanges).outstanding : 0;
...
function CustomerPortal({ currentUser, customers, invoices, payments, returns, exchanges, settings, offers, onLogout }) {
  const [viewing, setViewing] = useState(null);
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      {viewing && <InvoiceDetail invoice={viewing} settings={settings} onClose={() => setViewing(null)} />}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [u, s, c, p, i, pay, bk, ld, dr, ord, br, off, ret, exc, cn] = await Promise.all([
      ...
      setReturns(ret); setExchanges(exc); setCreditNotes(cn);
  ...
  const savePayment = (p) => persist.payments([...payments, p]);
  ...
  function createSalesReturn(data) {
    ...
    const code = "RET-" + String(counter).padStart(4, "0");
    const ret = { id: uid("ret"), code, status: "Returned", ...data };
  ...
  function createExchangeFn(data) {
    ...
    const code = "EXC-" + String(counter).padStart(4, "0");
    persist.exchanges([...exchanges, { id: uid("exc"), code, ...data }]);
  ...
  const pages = {
    dashboard: <Dashboard customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} returns={visibleReturns} exchanges={visibleExchanges} leads={visibleLeads} bookings={visibleBookings} />,
    customers: (
      <Customers
    ...
    invoices: (
      <Invoices
        ...
        returns={visibleReturns} exchanges={visibleExchanges}
    ...
    invoiceHistory: <InvoiceHistoryPage invoices={visibleInvoices} />,
    returns: <SalesReturnPage customers={visibleCustomers} invoices={visibleInvoices} returns={visibleReturns} currentUser={currentUser} onCreateReturn={createSalesReturn} />,
    exchange: <ExchangePage customers={visibleCustomers} products={products} invoices={visibleInvoices} exchanges={visibleExchanges} onCreateExchange={createExchangeFn} />,
    creditNotes: <CreditNotesPage creditNotes={visibleCreditNotes} onLinkInvoice={linkCreditNoteToInvoice} />,
    ledger: <LedgerView customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} returns={visibleReturns} exchanges={visibleExchanges} focusId={ledgerFocusId} setFocusId={setLedgerFocusId} settings={settings} />,
    payments: <Payments customers={visibleCustomers} payments={visiblePayments} savePayment={savePayment} />,
    bookings: (
      <Bookings
    ...
    reports: <Reports customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} returns={visibleReturns} exchanges={visibleExchanges} leads={visibleLeads} bookings={visibleBookings} />,
    assistant: <SalesAssistant customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} returns={visibleReturns} exchanges={visibleExchanges} />,
    ...
    settings: (
      <Settings
        ...
        allData={{ users, settings, customers, products, invoices, payments, bookings, leads, drivers, orders, branches, offers, returns, exchanges, creditNotes }}
    ...
    <div className="h-screen flex bg-slate-100 text-slate-900">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-slate-200">
          ...
          <div className="text-[10px] uppercase tracking-wide font-bold text-slate-400">{currentUser.role}</div>
        </div>

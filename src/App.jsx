...
function Invoices({ customers, products, drivers, invoices, payments, returns, exchanges, bookings, settings, currentUser, saveInvoice, updateInvoice, cancelInvoice, prefill, onClearPrefill, onBookingFulfilled, onOrderFulfilled, focusInvoiceId, setFocusInvoiceId, onGoToReturn, onGoToExchange }) {
...
function InvoiceHistoryPage({ invoices, auditLog }) {
  ...
  return (
    <div>
      ...
      <div className="text-xs text-slate-400 mb-4 max-w-2xl">
        Sales Return, Exchange, Delete Return, Delete Exchange, aur Invoice Status Change ki har action yahan record hoti hai — user, date, time aur reason ke saath.
      </div>
      <div className="bg-white border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [u, s, c, p, i, pay, bk, ld, dr, ord, br, off, ret, exc, cn, al] = await Promise.all([
      ...
      setReturns(ret); setExchanges(exc); setCreditNotes(cn); setAuditLog(al);
  ...
  // Feature 13: Audit Log helper. Records every Return, Exchange, Delete
  // Return, Delete Exchange, and Invoice Status Change with user/date/time/reason.
  ...
  function runGlobalSearch(query) {
    ...
    if (invoiceId) {
      ...
      alert("Koi invoice, return ya exchange is number se nahi mila.");
    }
  }
  ...
  const pages = {
    ...
    settings: (
      <Settings
        ...
        allData={{ users, settings, customers, products, invoices, payments, bookings, leads, drivers, orders, branches, offers, returns, exchanges, creditNotes, auditLog }}
        ...
        <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-slate-200 gap-4">
          ...
          <div className="flex-1 max-w-xs">
            <input
              ...
              placeholder="Search INV-xxxx / RET-xxxx-xx / EX-xxxx-xx..."
              value={globalSearch}

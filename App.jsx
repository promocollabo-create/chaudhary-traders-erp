...
/* ============================================================
   CHAUDHARY TRADERS — Browser ERP (Phase 1)
      ...
      <div className="bg-white w-full max-w-sm border-t-4 border-slate-900">
        ...
        <div className="p-6 pt-4">
          ...
          <div className="mt-4 text-[11px] text-slate-400 leading-relaxed">
            Default logins — admin / admin123 · staff / staff123.<br />Change these in Settings after first login.
          </div>
          {failCount >= 1 && (
...
function computeLedgerForCustomer(customer, invoices, payments) {
  const entries = [];
  invoices
...
function Dashboard({ customers, invoices, payments, leads, bookings }) {
  const outstandingTotal = useMemo(() => {
    return customers.reduce((sum, c) => sum + computeLedgerForCustomer(c, invoices, payments).outstanding, 0);
  }, [customers, invoices, payments]);

  const todaySales = useMemo(() => {
    const t = todayISO();
    return invoices.filter((i) => i.date === t).reduce((s, i) => s + i.total, 0);
  }, [invoices]);

  const monthSales = useMemo(() => {
    const m = todayISO().slice(0, 7);
    return invoices.filter((i) => i.date.startsWith(m)).reduce((s, i) => s + i.total, 0);
  }, [invoices]);
...
function Customers({ customers, invoices, payments, saveCustomer, deleteCustomer, openLedger, branches, currentUser }) {
  ...
  const rows = customers
    ...
    .map((c) => {
      const { outstanding } = computeLedgerForCustomer(c, invoices, payments);
...
function LedgerView({ customers, invoices, payments, focusId, setFocusId }) {
  ...
  const { entries, outstanding } = computeLedgerForCustomer(customer, invoices, payments);
...
function InvoiceForm({ customers, products, drivers, bookings, invoices, payments, prefill, onSave, onCancel, nextNumber }) {
  const [customerId, setCustomerId] = useState(prefill?.customerId || customers[0]?.id || "");
  ...
  const [rickshawRent, setRickshawRent] = useState(0);
  ...
  const previousOutstanding = selectedCustomer ? computeLedgerForCustomer(selectedCustomer, invoices, payments).outstanding : 0;
  ...
  return (
    <div>
        ...
        <div className="bg-slate-50 border border-slate-200 p-3 mb-3 text-sm">
          <div className="grid grid-cols-2 gap-y-1">
            ...
            <div className="col-span-2 pt-1 border-t border-slate-200 mt-1">
              <span className="text-slate-500">Current Outstanding (before this invoice):</span>{" "}
      ...
      <div className="flex gap-2 mt-4">
        <Btn onClick={submit}>Save Invoice {nextNumber}</Btn>
        <Btn variant="ghost" onClick={onCancel}>Cancel</Btn>
      </div>
    </div>
  );
...
function Invoices({ customers, products, drivers, invoices, payments, bookings, settings, saveInvoice, prefill, onClearPrefill, onBookingFulfilled, onOrderFulfilled }) {
  ...
  return (
    <div>
          ...
          <tbody>
            ...
            {sorted.map((inv) => (
              <tr key={inv.id} className="border-t border-slate-100 cursor-pointer hover:bg-slate-50" onClick={() => setViewing(inv)}>
                ...
                <td className="px-4 py-2 text-slate-500">{fmtDate(inv.date)}</td>
                <td className="px-4 py-2 text-right font-bold">{fmtMoney(inv.total)}</td>
                <td className="px-4 py-2 text-right text-red-600 font-bold">{inv.balanceDue > 0 ? fmtMoney(inv.balanceDue) : "-"}</td>
      ...
      {showForm && (
        <Modal title={prefill ? `New Invoice — from ${prefill.sourceCode}` : "New Invoice"} onClose={closeForm} wide>
          <InvoiceForm
            customers={customers}
            products={products}
            drivers={drivers}
...
function Reports({ customers, invoices, payments, leads, bookings }) {
  const [from, setFrom] = useState(todayISO().slice(0, 8) + "01");
  const [to, setTo] = useState(todayISO());

  const inRange = invoices.filter((i) => i.date >= from && i.date <= to);
  const salesTotal = inRange.reduce((s, i) => s + i.total, 0);
  const collected = payments.filter((p) => p.date >= from && p.date <= to).reduce((s, p) => s + p.amount, 0);
...
function SalesAssistant({ customers, invoices, payments }) {
  ...
  const outstanding = customer ? computeLedgerForCustomer(customer, invoices, payments).outstanding : 0;
...
function CustomerPortal({ currentUser, customers, invoices, payments, settings, offers, onLogout }) {
  const [viewing, setViewing] = useState(null);
            ...
            <tbody>
              ...
              {myInvoices.map((inv) => (
                <tr key={inv.id} className="border-t border-slate-100 cursor-pointer hover:bg-slate-50" onClick={() => setViewing(inv)}>
                  ...
                  <td className="px-4 py-2 text-slate-500">{fmtDate(inv.date)}</td>
                  <td className="px-4 py-2 text-right font-bold">{fmtMoney(inv.total)}</td>
                  <td className="px-4 py-2 text-right text-red-600 font-bold">{inv.balanceDue > 0 ? fmtMoney(inv.balanceDue) : "Paid"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [u, s, c, p, i, pay, bk, ld, dr, ord, br, off] = await Promise.all([
  ...
  function saveInvoice(inv) {
    ...
    if (inv.paymentReceived > 0) {
      persist.payments([...payments, {
        ...
        date: inv.date, amount: inv.paymentReceived, method: "Cash", note: `Against ${inv.number}`,
      }]);
    }
  }
  ...
  const pages = {
    dashboard: <Dashboard customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} leads={visibleLeads} bookings={visibleBookings} />,
    customers: (
      <Customers
    ...
    invoices: (
      <Invoices
        ...
        bookings={visibleBookings} settings={settings} saveInvoice={saveInvoice}
    ...
    ledger: <LedgerView customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} focusId={ledgerFocusId} setFocusId={setLedgerFocusId} />,
    ...
    reports: <Reports customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} leads={visibleLeads} bookings={visibleBookings} />,
    assistant: <SalesAssistant customers={visibleCustomers} invoices={visibleInvoices} payments={visiblePayments} />,
    ...
    settings: (
      <Settings
        ...
        allData={{ users, settings, customers, products, invoices, payments, bookings, leads, drivers, orders, branches, offers }}
